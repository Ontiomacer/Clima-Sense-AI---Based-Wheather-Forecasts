...
// Use deployed Vercel API endpoints
const VERCEL_API_BASE = 'https://clima-sense-ai-based-wheather-forec.vercel.app';
...
// Fetch AI forecast data from Vercel API
async function fetchAIForecast(lat: number, lon: number, location: string) {
  try {
    const response = await axios.post(`${VERCEL_API_BASE}/api/ai-forecast`, {
      lat,
      lon,
      location,
    }, {
...
// Fetch agricultural analysis from Vercel API
async function fetchAgriAnalysis(query: string) {
  try {
    const response = await axios.post(`${VERCEL_API_BASE}/api/agri-analysis`, {
      text: query,
    }, {
      timeout: 30000,
    });
    ...
    tools: [
      {
        ...
        description: 'Get comprehensive weather forecast for any Indian state or city. Provides 180-day AI-powered predictions including temperature, rainfall, and humidity with confidence intervals. Data is fetched from ClimaSense deployed API.',
        inputSchema: {
          type: 'object',
          properties: {
            location: {
      ...
      },
      {
        ...
        description: 'Get AI-powered agricultural recommendations based on crop conditions, soil status, weather patterns, or farming queries. Powered by AgriBERT model via ClimaSense API.',
        inputSchema: {
          type: 'object',
          properties: {
            query: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    if (name === 'get_weather_forecast') {
      ...
      if (!forecastData || !forecastData.success) {
        return {
          content: [
            {
              type: 'text',
              text: `❌ Unable to fetch weather data for ${detected.state}. The ClimaSense API may be temporarily unavailable. Please try again in a moment.`,
            },
          ],
        };
      }
      ...
      response += `**Data Source:** ClimaSense AI (NASA POWER + OpenWeather)\n\n`;
      ...
      response += `- **Powered by:** ClimaSense AI\n`;
    ...
    if (name === 'get_agricultural_advice') {
      ...
      if (!aiResponse) {
        return {
          content: [
            {
              type: 'text',
              text: `❌ Unable to get agricultural advice. The ClimaSense API may be temporarily unavailable. Please try again in a moment.`,
            },
          ],
        };
      }
      ...
      response += `**Powered by:** ClimaSense AI (AgriBERT)\n`;
...
async function main() {
  ...
  console.error('ClimaSense Remote MCP Server running on stdio');
  console.error('Connected to: https://clima-sense-ai-based-wheather-forec.vercel.app');
}
